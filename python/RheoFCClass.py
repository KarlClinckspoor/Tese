...
class Fitter:
    ...
    def automatic_nl_fitting(self, save=True):
        ...
        for first_point in range(0, max_range, 1):
            ...
            Eta_arr = np.array(self.Eta[first_point:])
            nonlinear_has_error = ''
            print(f'Point {first_point} of max {self.max_fp}')
            try:
    ...
    def plot_error_graphs(self): # todo: If it has both plots, make them side by side
        ...
        if self.nl_done and self.lin_done:
            fig, [axn, axl] = plt.subplots(ncols=2, nrows=1, figsize=(9, 4))
        ...
        if self.nl_done:
            ...
            axn.errorbar(x, y, yerr=yerr, capsize=2)
            ...
            # fig.text(TEXT_FILENAME_X, TEXT_Y - 0.005, self.filename, size='small')
            ...
            TEXT_FILENAME_X = 0.55  # Changes it to half in case a linear plot is done.
            axn.set(xlabel=r'$\dot{\gamma}$', ylabel=r'$\eta$')
        ...
        if self.lin_done:
            ...
            param_text = f"int = {self.int:.3f}+/-{self.int_err:.3f}"

            total_text = f'{self.filename}\n{model_param_names}\n{param_text}' # $R^2$={round(self.l_R2, 2)}
            anchored_text = AnchoredText(total_text, loc=3, frameon=True, prop={'fontsize':'small'})
            ...
            # fig.text(TEXT_FILENAME_X, TEXT_Y - 0.090, param_errors, size='small')
            #fig.set_size_inches(6, 4)  # todo: check if this is still necessary
            axl.set(xlabel=r'$\dot{\gamma}$', ylabel=r'$\eta$')

        plt.tight_layout()

        if self.settings.SAVE_GRAPHS:

            fig.savefig(self.filename[:-4] + '.pdf')
            print('Figure saved.')
