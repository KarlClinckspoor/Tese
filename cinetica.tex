...
%	\end{figure}
	...
	Mediante isso, foram realizados estudos de cinética misturando-se duas soluções, uma contendo somente NaSal 1,5 \mM{} e outra contendo tanto NaSal 1,5 \mM{} quanto \TTAB{} 1,7 \mM. Essa concentração de \TTAB{} está longe da região do mínimo, com baixa viscosidade. Quanto misturada à outra solução, o \TTAB{} seria diluído pela metade, atingindo a concentração de 0,85 \mM{}, próximo ao mínimo, portanto ainda viscosa, mas mais fluorescente do que as micelas no mínimo. A figura \ref{fig:itc_experimento_cinetica} ilustra a mistura que irá ocorrer.
	...
	Como resultado de cada análise, um mapa de cor é obtido, contendo os espectros medidos em y, o tempo em x, e as cores correspondendo à intensidade do sinal. Um mapa de cor padrão se encontra na figura XXXX.
	
	% todo: colocar um mapa
	
	Desse mapa, foi retirado a variação da intensidade em 411 nm, o máximo do sinal, em função do tempo. Cada sinal é extremamente ruidoso, devido ao baixo tempo de integração (5 ms). % todo: verificar se esse é o tempo mesmo.
	Cada curva foi alisada pelo método de Savitzky-Golay, utilizando uma janela variável dependendo da região da curva. Antes do ponto de injeção, caracterizado por um salto na intensidade, foi utilizada uma janela pequena. Depois do ponto de injeção, foi utilizado uma janela muito maior, para aumentar a intensidade de alisamento. A figura XXXX mostra uma curva não alisada e alisada.
	
	Em determinada parte dos experimentos, observou-se que a intensidade de fluorescência diminuía em função do tempo, somente nas amostras contendo surfactante. Por essa razão, foram realizados experimentos utilizando NaSal 3,0 \mM{} em água, e em soluções de sacarose 4\% e 10\% m/m, que simulariam a viscosidade das micelas gigantes. Após a mistura, a concentração de NaSal atingiria 1,5 \mM, equivalente aos outros experimentos. Notou-se que não havia uma queda na intensidade de fluorescência, como mostrado na figura XXXX.
	
	Além disso, as injeções realizadas diretamente na cela de amostra contendo a corrida anterior também mostraram variação, mostrando que os estados iniciais e finais dessas misturas são diferente, e que ocorre uma evolução do sistema micelar durante o processo. Porém, as escalas de tempo não são da ordem de \(<\) 100 ms, como nos experimentos de cinética, mas sim de centenas de ms, atingindo até segundos.
	
	As regiões de decaimento após a mistura foram ajustadas por quatro modelos de decaimento exponencial, e o melhor ajuste foi o modelo monoexponencial (Eq. \ref{eqn:decaimento_exponencial}). Os valores da constante de decaimento \(t_1\) foram comparados, e observou-se uma clara diferenciação entre os experimentos com e sem \TTAB, mostrando que os efeitos observados são dependentes das micelas gigantes.
	
	Visto isso, foram realizados experimentos de stopped-flow em outro equipamento, utilizando tempos de integração muito maiores, para ver se o mesmo comportamento de queda de fluorescência poderia ser observado. Os resultados desses experimentos estão na figura XXX. Porém, nenhuma variação foi vista. 
	
	
	% Todo: colocar sinal no máximo
	
	% todo: colocar gráfico alisado
