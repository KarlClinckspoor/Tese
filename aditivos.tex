...
%		\begin{equation}
%			\dfrac{n^2 - 1}{n^2 + 2}\dfrac{1}{\rho} = \dfrac{4\pi N_A}{3} \alpha_0
%			\label{eqn:lorenz_lorentz}  % todo: ref olson1975
%		\end{equation}
		
		\noindent onde \(n\) é o índice de refração, \(\rho\) é a densidade molar, \(N_A\) é o número de Avogadro e \(\alpha_0\) é a polarizabilidade eletrônica molecular isolada.
		...
		A constante dielétrica \(\varepsilon\) é um parâmetro relacionado à polaridade do solvente. Quanto maior for a constante dielétrica, mais fácil é a separação de cargas no solvente pois menor é sua atração (eq. \ref{eqn:U_eletrostatica}). 
...
%		Interessantemente, a constante dielétrica também está relacionada à polarizabilidade molecular total (\(\alpha\)), através da equação de Clausius-Mossotti \ref{eqn:clausius_mossotti}.
%		
%		\begin{equation}
...
%		\end{equation}
%		
%		Devido à essa relação, é possível correlacionar o índice de refração à constante dielétrica, obtendo a eq. \ref{eqn:n_epsilon}. Neste caso é necessário notar a diferença entre a constante dielétrica e permissividade elétrica. A constante dielétrica é medida onde a frequência do campo elétrico é zero, já a permissividade é o nome dado para valores medidos em frequências diferentes. A relação \ref{eqn:n_epsilon} necessita que uma frequência seja estabelecida.  % todo: reler partes do Israelachvili e prestar atenção na polarizabilidade total e eletrônica \alpha_0
%		
%		\begin{equation}
%			n = \varepsilon_r^{1/2}
%			\label{eqn:n_epsilon}
%		\end{equation}  % todo: pensar sobre essa equação e se ela realmente vale para as coisas aqui. Não parece ser tão assim
		...
		Porém, as equações \ref{eqn:lorenz_lorentz} e \ref{eqn:clausius_mossotti} consideram as moléculas num meio como sendo esferas com constantes dielétricas diferentes daquelas do meio. Comparando-se os valores calculados por essas equações e os valores medidos, há discrepâncias bastante altas para moléculas altamente polares.
		
		Esse tipo de equação não considera interações além daquelas oriundas totalmente da polarização eletrônica e dipolar. Alguns solventes conseguem deslocar cargas em comprimentos maiores que o tamanho molecular. Na água, acredita-se que sua alta constante dielétrica se deve também à movimentação eletrônica e protônica através de redes de ligação de hidrogênio. % ref: israelachvili2010.
