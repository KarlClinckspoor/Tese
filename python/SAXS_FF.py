...
def wormlike_chains(q, scale, d_head, Rcore, rho_rel, sigma, back, cont_l, kuhn_l, eps, dist_cq, nu_ppa,
    ...
    FX = (((rho_rel * Rout ** 2 * 2 * jv(1, Rout * q) / (q * Rout)) -
          ...
          np.exp((-1/2) * q ** 2 * sigma ** 2))  # Região interna

    FOX = rho_rel * Rout ** 2 - (rho_rel + 1) * Rcore ** 2  # Região externa

    # Integração para obter fator forma do cilindro
    cyl_int = 0  # SUM
    npoints = 50
    step = 0.5 * np.pi / npoints

    EPSO = (Rcore * eps + d_head) / Rout  # Densidade externa
    ...
    cyl_int = cyl_int * step * 2 / np.pi  # Normaliza integração
    ...
    Rout = abs(d_head) + abs(Rcore)  # Utiliza valor absoluto para ignorar artefatos do ajuste
