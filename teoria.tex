...
%				\end{subfigure}
	...
	\chapter{SAXS}
		...
		O espalhamento de luz é um fenômeno que ocorre devido à interação da luz com os elétrons de um átomo ou grupo de átomos. O feixe incidente excita os elétrons de modo que eles comecem a oscilar na mesma frequência do feixe incidente. Quando o feixe incidente é removido, os elétrons excitados emitem fótons do mesmo comprimento de onda incidente (chamado de espalhamento elástico), em qualquer direção. As ondas emitidas interagem de maneira construtiva e destrutiva, dependendo de sua posição relativa. Isso resulta em padrões de espalhamento diferentes, dependendo da disposição espacial dos centros espalhadores.
		
		Experimentalmente, isso é realizado utilizando-se uma fonte de raios-X, que é colimada e passada por um monocromador. O feixe incidente, idealmente monocromático e paralelo, incide na amostra e é espalhado, atingindo um detector. As intensidades de radiação que atingem cada pixel do detector são armazenadas como uma matriz \((m,n)\), que pode ser representada como uma imagem de cor falsa.
		
		 A figura \ref{fig:esquema_saxs} ilustra os feixes incidentes e espalhados, junto com o vetor de espalhamento \q{} (Eq. \ref{eqn:vetor_espalhamento_vetorial}) e a imagem de espalhamento obtida.
		...
		Caso a imagem formada seja radialmente simétrica, o que ocorre em amostras isotrópicas, o sinal pode ser integrado em \(\phi\), e o vetor de espalhamento é representado pela eq. \ref{eqn:vetor_espalhamento}.
